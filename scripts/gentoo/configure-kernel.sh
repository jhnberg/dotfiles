...
log "Reading /proc/config.gz"
zcat /proc/config.gz > "${KERNEL_CONFIG}"

CPU_VENDOR=$(lscpu --json | jq -r '.lscpu[] | select(.field == "Vendor ID:") | .data')

# Track the kernel config options and set them at the end.
# The enable(), module() and disable() functions are utilities to
...
kconfig_system() {
    log "Configuring system specific options"
    ...
    then
        log "- Disabling Intel specific options"
    ...
    then
        log "- Disabling AMD specific options"
...
kconfig_compress() {
    log "Configuring compression options"
...
kconfig_filesystems() {
    log "Configuring file system options"
    ...
    disable CONFIG_XFS_SUPPORT_V4
    disable CONFIG_XFS_SUPPORT_ASCII_CI
...
kconfig_storage() {
    log "Configuring storage options"
...
kconfig_virtualisation() {
    log "Configuring virtualisation options"
...
# tricky to manage. So we set them here.
...
# Apply the config options in the following:
#
# 1. disable
# 2. module
# 3. enable
#
# This ensures that enabled options takes precedence over modules, and
# modules over disables options.
echo  ${KCONFIG_N[@]}                                                 \
    | xargs -rn1                                                      \
    | xargs -rn1 echo --disable                                       \
    | xargs -r ${KERNEL_SRC}/scripts/config --file "${KERNEL_CONFIG}"
echo -n ${KCONFIG_M[@]}                                               \
    | xargs -rn1 echo                                                 \
    | xargs -rn1 echo --module                                        \
    | xargs -r ${KERNEL_SRC}/scripts/config --file "${KERNEL_CONFIG}"
echo -n ${KCONFIG_Y[@]}                                               \
    | xargs -rn1 echo                                                 \
    | xargs -rn1 echo --enable                                        \
    | xargs -r ${KERNEL_SRC}/scripts/config --file "${KERNEL_CONFIG}"
